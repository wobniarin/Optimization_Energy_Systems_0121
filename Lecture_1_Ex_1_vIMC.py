...
# You will have to have pyomo and gurobi or any other AMPL compatable solver installed in the same path.
from pyomo.environ import *
import pyomo.environ as pyo
from pyomo.opt import SolverFactory
import gurobipy as gp # This is not needed, since we are using pyomo for writing the script 
import pandas as pd
import numpy as np


# Importing exel sheet data into data frames
...
#Define Sets of Indecies, this will not work on a problem where multiple generators are connected to the same bus.
...
#This constraint and all other non-negativity constraints can be removed by adding a zero to inf bound to the variables (e.g. model.pd=Var(D,bound(0,inf)))
...
def NodBal_rule(mod,n):
    return mod.pd[n]-mod.pg[n] == sum((mod.d[i-1]-mod.d[k-1])/FullBranch.loc[x,"X"] for x,(i,k) in enumerate(zip(FullBranch["From"], FullBranch["To"])) if i == n+1)
